...
uint8_t USBHub::Init(uint8_t parent, uint8_t port, bool lowspeed, USB_DEVICE_DESCRIPTOR* udd) {
        uint8_t buf[32];
        ...
        if(!rcode) {
                cd_len = min(ucd->wTotalLength, sizeof(buf));
        ...
        // It only intended for the usb protocol sniffer to properly parse hub-class requests.
        {
                ...
                rcode = pUsb->getConfDescr(bAddress, 0, min(buf[0], sizeof(buf2)), 0, buf2);
        ...
        //bInitState = 0;
        //USBTRACE("...OK\r\n");
        Serial1.println("HUB REGISTERED");
        return 0;
