        ...
        switch(usb_task_state) {
                ...
                case USB_STATE_CONFIGURING:
                        ...
                        if(rcode) {
                                ...
                                if (rcode == USB_ERROR_RESET_ENUMERATION)
                                {
                                        usb_task_state = USB_STATE_CONFIGURING;
                                }
                        } else
...
uint8_t USB::AttemptConfig(uint8_t driver, uint8_t parent, uint8_t port, bool lowspeed, USB_DEVICE_DESCRIPTOR* udd) {
        //printf("AttemptConfig: parent = %i, port = %i\r\n", parent, port);
        uint8_t retries = 0;
        uint8_t rcode;

again:
        rcode = devConfig[driver]->Init(parent, port, lowspeed, udd);
...
uint8_t USB::Configuring(uint8_t parent, uint8_t port, bool lowspeed) {
        ...
        rcode = getDevDescr(0, 0, sizeof(USB_DEVICE_DESCRIPTOR), (uint8_t *)&udd);
        if (rcode)
        {
                return rcode;
        }
        ...
        for (devConfigIndex = 0; devConfigIndex < USB_NUMDEVICES; devConfigIndex++)
        {
                ...
                if (devConfig[devConfigIndex]->GetAddress())
                        continue; // consumed
                rcode = AttemptConfig(devConfigIndex, parent, port, lowspeed, &udd);

                if(rcode == hrSUCCESS || rcode == USB_ERROR_RESET_ENUMERATION)
                {
                        break;
                }
        }
